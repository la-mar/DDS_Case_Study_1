...
Authors: <strong> Peter Flaming </strong> | <strong> Matthew Trevathan </strong> | <strong> Quinton Nixon </strong> | <strong> Brock Friedrich </strong>
<hr />
...
ggplot2::theme_set(ggplot2::theme_bw()) # set global ggplot theme

# define colors for plot themes
...
kable(breweries_dups)

```

4) Correct duplicates

    a) City name "Menominie" misspelled as "Menominee"
...
```
    ...
    b) Marquette is not a city name in Massachusetts. Changed to Michigan based on other records existing for Marquette, MI, in addition to validating the location of the city on Google Maps.
...
```
    ...
    c) Merge duplicates into single records on name + city + state
...
```
    ...
    d) Create new brewery_id for corrected duplicates
...
```
    ...
    e) Update original breweries dataset with corrections and de-duped records
...
print(summarytools::dfSummary(breweries_clean), method = 'render')
...
print(summarytools::dfSummary(beer_clean), method = 'render')
...
abv_stats <- select(merged_data, abv) %>% #select columns
                   na.omit() %>% # omit nulls
                   dplyr::summarize_all(funs(count=n(), 
                                             min(abv), 
                                             max(abv), 
                                             mean(abv), 
                                             median(abv), 
...
ibu_stats <- select(merged_data, ibu) %>% #select columns
                   na.omit() %>% # omit nulls
                   dplyr::summarize_all(funs(count=n(), 
                                             min(ibu), 
                                             max(ibu), 
...
* Assumptions:
    + Continuity of data: <font color="green"> &#x2713; </font>
    + Paired observations: <font color="green"> &#x2713; </font>
    + Data has linear relationship: <font color="green"> &#x2713; </font> 
    + No significant outliers:  <font color="red"> &#x2717; </font>
    + Normality:<font color="red"> &#x2717; </font>
...
# Significance test
spear_test_result <- cor.test(styles$ibu, styles$abv, method = "spearman", conf.level = alpha, exact=FALSE)
...
People all around the world have been drinking beef for centuries.  Beer has been used for refreshment, for refueling, and even for remittance.  A survey of craft brewers in the US shows that beer is still a favorite, mainstream beverage.  

We found that there is at least one craft brewery in every state with an average brewery to citizen ratio of 1:125,000.  There are more than `r (select(styles, style) %>% na.omit() %>% summarize_all(funs(ct = n_distinct(.))))[1,] ` varieties of beer being brewed with a range of strength (alcohol content and bitterness).  The most popular craft beer being brewed is Y with an ABV of Z and an IBU of AA which puts it at the BB percentile on both scales. 

We determined that there is a distinct positive correlation between alcohol content and bitterness.

With so many breweries and so many styles of beer, one might expect that all of the beer bases are covered…one would be wrong.  There are a handful of beer types that are statistically under-represented.  An enterprising brewer might want to consider exploiting these overlooked beers to become a major producer of a niche style.
