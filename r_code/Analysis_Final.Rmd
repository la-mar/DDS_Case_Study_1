...
abv_stats <- select(merged_data, abv) %>% #select columns
                   na.omit() %>% # omit nulls
                   dplyr::summarize_all(funs(count=n(), min(abv), max(abv), mean(abv), median(abv), sd(abv)))

ibu_stats <- select(merged_data, ibu) %>% #select columns
                   na.omit() %>% # omit nulls
                   dplyr::summarize_all(funs(count=n(), min(ibu), max(ibu), mean(ibu), median(ibu), sd(ibu)))
