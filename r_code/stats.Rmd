...
isnull <- beer_data %>% filter(!is.na(IBU)) %>% distinct(Beer_ID, Style, IBU, ABV, Ounces) %>% arrange(Style)
...
ggplot((styles %>% drop_na(IBU)), aes(x="", y=IBU)) +
      geom_point(aes(fill = ifelse((IBU %in% ibu_outliers),"Outlier","Valid")), size = 5, shape = 21, position = position_jitter())+
